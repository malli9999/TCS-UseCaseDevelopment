...
public DemoAccountModel getDeatils(@RequestBody DemoUserSessionId sess) {
	...
	if(sess.getUserName().equals(user.getUserName()) && user.getCustomer_Session_Id().equals(sess.getSession())) {
		...
		demo = restTemplate.getForObject("http://Demo-AccountModel/acc/"+ sess.getUserName(), DemoAccountModel.class);
...
public DemoTransactionModel getTransDeatils(@RequestBody DemoUserTrans trans) {
	...
	transmodel = restTemplate.getForObject("http://Demo-AccountModel/transaction/"+ trans.getAccount(), DemoTransactionModel.class);
	...
	return transmodel;
		...
		System.out.println("The sessin is inactive please log ihn again");
...
public DemoRewardsModel getRewards(@PathVariable("username") String username){
	...
	rewmodel = restTemplate.getForObject("http://Demo-AccountModel/rewards/" + username, DemoRewardsModel.class);
	return rewmodel;
}
